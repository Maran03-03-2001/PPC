...
const FreePlansWithProperties = () => {
    ...
    if (window.confirm(`Are you sure you want to delete PPC ID: ${ppcId}?`)) {
      try {
        const response = await fetch(
          `${process.env.REACT_APP_API_URL}/delete-free-property/${ppcId}`,
          {
            method: "PUT",
          },
        );
  ...
  const handleUndoDelete = async (ppcId) => {
    if (
      window.confirm(
        `Are you sure you want to undo delete for PPC ID: ${ppcId}?`,
      )
    ) {
      try {
        const response = await fetch(
          `${process.env.REACT_APP_API_URL}/undo-delete-free-property/${ppcId}`,
          {
            method: "PUT",
          },
        );
          ...
          <tbody>
            {filteredData.map((property, index) => (
              <tr key={property.ppcId}>
                ...
                <td>
                  {new Date(property.user?.billCreatedAt).toLocaleString()}
                </td>
                <td>
                  {new Date(property.user?.planExpiryDate).toLocaleDateString()}
                </td>
                <td>
                  {property.isDeleted ? (
                    <button
